class TransportationProblem:
    ...
    def display_full_transportation_problem_with_proposal(self, aesthetic_spaces=1, balas_hammer=None, penalties=None, penalty_equalities=None):
        ...
        for i in range(self.nb_suppliers):
            for j in range(self.nb_customers):
                ...
                size_char = len(str(self.costs_matrix[i][j])) + 3 + len(str(self.transport_proposal_matrix[i][j]))  # + 3 for len(' / ')
                if size_char > max_char_size:
                    max_char_size = size_char
    ...
    def north_west(self):
        ...
        while to_complete != [0 for _ in range(self.nb_customers)]:
            ...
            # We got two cases, either supply < order and we put all the supply in the proposal cell,
            # on the other case we put all the amount needed for the order in the cell
            self.transport_proposal_matrix[i][j] = available[i] if available[i] < to_complete[j] else to_complete[j]

            # We update our supply and demand accordingly to the modification we've done
            available[i] = available[i] - self.transport_proposal_matrix[i][j]
            to_complete[j] = to_complete[j] - self.transport_proposal_matrix[i][j]

            # Following depletion of the demand or the supply, we move to the column to its right or the row under
            if to_complete[j] == 0:
                j += 1
            else:
                i += 1
    ...
    def balas_hammer(self, with_display=True):
        ...
        while to_complete != [0 for _ in range(0, len(to_complete))]:
            ...
            for i in range(0, len(penalties_row)):
                min_c = [float('inf'), float('inf')]  # Infinity is of course an easy minimum to dislodge
                for j in range(len(self.orders)):  # Goes through whole row
                    # if we find a cost lower than our minimum (excludes used rows and columns marked by a -1 penalty),
                    # we move the former minimum to the second spot, becoming our 2nd lowest cost
                    if self.costs_matrix[i][j] <= min_c[0] and penalties_row[i] != -1 and penalties_col[j] != -1:
                        min_c[1] = min_c[0]
                        min_c[0] = self.costs_matrix[i][j]
                # And to be sure to have the 2nd lowest minimum in min[1] we check again (needed):
                for j in range(len(self.orders)):
                    # exclude the smallest minimum
                    if self.costs_matrix[i][j] <= min_c[1] and self.costs_matrix[i][j] != min_c[0] and penalties_row[i] != -1 and penalties_col[j] != -1:
                        min_c[1] = self.costs_matrix[i][j]  # Assign penultimate to second minimum
                # To avoid inf - inf producing a nan, we replace by -1 in some cases
            ...
            # in similar way than for each row
            ...
            # and be able to handle equality cases.
            penalty_equalities = []
                ...
                if p[0] == "col":
                    for s in range(self.nb_suppliers):
                        if cheapest_cell_index is None:
                            if penalties_row[s] != -1:
                                ...
                                # Assigns as much supply as possible
                            ...
                            # Finds cell with cheapest cost (excluding -1 cols)
                    ...
                    for c in range(self.nb_customers):
                        if cheapest_cell_index is None:
                            ...
                            # Finds cell with cheapest cost (excluding -1 rows)
            ...
            if chosen_penalty[0] == "col":  # If the chosen max penalty is a column
                ...
                # Assigns as much supply as possible to that cheap cell
                ...
                # fill max quantity possible in the same way that for col
